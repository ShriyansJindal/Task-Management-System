        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 4:
                    ...
                    taskManager.getTasks().forEach(n-> System.out.println(n));
                    System.out.println();
                    break;
                ...
                case 5:
                    ...
                    status = Status.valueOf(sc.nextLine().toUpperCase());
                    taskManager.filterTasksByStatus(status).forEach(System.out::println);
                    break;

                case 6:
                    System.out.print("Enter priority to filter (HIGH, MEDIUM, LOW): ");
                    priority = Priority.valueOf(sc.nextLine().toUpperCase());
                    taskManager.filterTasksByPriority(priority).forEach(System.out::println);
                    break;

                case 7:
                    System.out.print("Enter due date to filter (YYYY-MM-DD): ");
                    dueDate = LocalDate.parse(sc.nextLine());
                    taskManager.filterTasksByDueDate(dueDate).forEach(System.out::println);
                    break;
